    ...
    cron.schedule('5 8 * * *', async () => {
        ...
        try {
            ...
            const schedules = await pool.query(
                `SELECT * FROM survey_schedules WHERE is_active = true AND next_due_date <= $1`,
                [today]
            );
            console.log(`[CRON] ${schedules.rows.length} survey schedule(s) due`);
            ...
            for (const schedule of schedules.rows) {
                ...
                for (const pledge of pledges.rows) {
                    try {
                        ...
                        const inst = await pool.query(
                            `INSERT INTO survey_instances (schedule_id, pledge_id, due_date)
                ...
                await pool.query(
                    `UPDATE survey_schedules SET next_due_date = $1 WHERE id = $2`,
