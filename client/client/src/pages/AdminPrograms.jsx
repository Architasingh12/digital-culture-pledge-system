...
import { Plus, Calendar, Target, LayoutGrid, Building2 } from 'lucide-react';
...
const AdminPrograms = () => {
    ...
    const handleOpenModal = () => {
        setNewProgram({ title: '', description: '', start_date: '', end_date: '', company_id: '' });
        setShowModal(true);
    };

    const handleCreateProgram = async (e) => {
        e.preventDefault();
        if (!newProgram.company_id) {
                            ...
                            {programs.map(p => (
                                <motion.div key={p.id} variants={itemVariants} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors flex justify-between items-center group">
                                    <div>
                                        ...
                                        {p.company_name && (
                                            <div className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 bg-purple-50 dark:bg-purple-900/10 text-purple-700 dark:text-purple-400 text-xs font-bold rounded-lg border border-purple-200 dark:border-purple-800/50">
                            ...
                            <form onSubmit={handleCreateProgram} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        value={newProgram.company_id}
                                        onChange={e => setNewProgram({ ...newProgram, company_id: e.target.value })}
                                    >
